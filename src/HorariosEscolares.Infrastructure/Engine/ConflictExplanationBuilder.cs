...
public static class ConflictExplanationBuilder
{
        ...
        GenerationContext context)
    {
        ...
        int maxSlots = context.School.SlotsPerDay * 5;
        if (teacherSlotsUsed >= maxSlots - 2)
        {
            return new ConflictExplanation
            {
                Type = Domain.Entities.ConflictType.Teacher,
                Severity = Domain.Entities.ConflictSeverity.Error,
        ...
        if (session.RequiredClassroomType.HasValue)
        {
            return new ConflictExplanation
            {
                Type = Domain.Entities.ConflictType.Classroom,
                Severity = Domain.Entities.ConflictSeverity.Error,
        ...
        if (weeklyAssigned > 0)
        {
            return new ConflictExplanation
            {
                Type = Domain.Entities.ConflictType.Normative,
                Severity = Domain.Entities.ConflictSeverity.Warning,
